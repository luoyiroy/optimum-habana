...
class GaudiXyzRotaryEmbedding(torch.nn.Module):
    def __init__(
        self,
        dim=None,
...
class TPGaudiXyzMLP(GaudiXyzMLP, TPModule):
    def __init__(
        self,
        config,
        ...
        GaudiXyzMLP.__init__(self, self.config)
    ...
    @staticmethod
    def import_module(glu: GaudiXyzMLP, group: ProcessGroup) -> "TPGaudiXyzMLP":
        config = copy.deepcopy(glu.config)
        config.hidden_grow_factor = glu.config.intermediate_size / glu.config.hidden_size
        tp_glu = TPGaudiXyzMLP(config=config, group=group)
        return tp_glu

    def pre_mlp_forward(self, x):
        out_par = GaudiXyzMLP.pre_mlp_forward(self, x)
        return reduce_from_tensor_model_parallel_region(out_par)


def gaudi_xyz_repeat_kv(
    ...
    def pre_attn_forward(
        ...
        **kwargs,
        ...
        if use_flash_attention and FusedSDPA is not None:
            ...
            query_states, key_states, value_states, attention_mask = gaudi_xyz_repeat_kv(
...
class TPGaudiXyzAttention(GaudiXyzAttention, TPModule):
    def __init__(
        self,
        config: XyzConfig,
        ...
        GaudiXyzAttention.__init__(self, self.config, layer_idx)
    ...
    @staticmethod
    def import_module(mha: GaudiXyzAttention, layer_idx, group: ProcessGroup) -> "TPGaudiXyzAttention":
        tp_mha = TPGaudiXyzAttention(config=mha.config, layer_idx=layer_idx, group=group)
        return tp_mha
    ...
    def pre_attn_forward(
        ...
        **kwargs,
        ...
        hidden_states, attn_weights, present_key_value = GaudiXyzAttention.pre_attn_forward(
            self,
...
class GaudiXyzModel(XyzModel):
    ...
    def __init__(self, config: XyzConfig):
        ...
        for layer_idx in range(config.num_hidden_layers):
            layer = GaudiXyzDecoderLayer(config, layer_idx)
